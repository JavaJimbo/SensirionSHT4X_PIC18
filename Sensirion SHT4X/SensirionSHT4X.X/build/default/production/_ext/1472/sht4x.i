...
# 47 "../sht4x.h" 2
# 69 "../sht4x.h"
int16_t sht4x_probe(uint32_t *serial);
# 85 "../sht4x.h"
int16_t sht4x_measure_blocking_read(float* floTemperature, float* floHumidity);
# 94 "../sht4x.h"
...
# 45 "../sht4x.c" 2
# 55 "../sht4x.c"
static uint8_t sht4x_cmd_measure = 0xFD;
static uint16_t sht4x_cmd_measure_delay_us = 10000;

int16_t sht4x_measure_blocking_read(float* floTemperature, float* floHumidity)
{
    int16_t errors, retries;
    int32_t temperature, humidity;
    float temp;

    errors = sht4x_measure();
    printf(" >");
    if (errors)
    {
        printf("\rsht4x_measure errors");
        return errors;
    }
    ...
    do {
        DelayMs(10);
        errors = sht4x_read(&temperature, &humidity);
        if (errors)
        {
            retries++;
            if (retries > 40) break;
    ...
    else
    {
        ...
        *floTemperature = temp / 1000.0f;
        temp = (float)humidity;
        *floHumidity = temp / 1000.0f;
    }
    return errors;
}

int16_t sht4x_measure(void) {
    return sensirion_i2c_write(0x44, &sht4x_cmd_measure, 1);
}

int16_t sht4x_read(int32_t* temperature, int32_t* humidity)
{
    uint16_t words[2];
    int16_t errors = sensirion_i2c_read_words (0x44, words, (sizeof(words) / 2));
...
int16_t sht4x_probe(uint32_t *serial)
{
    ...
    errors = sht4x_read_serial(&sensorSerialNumber);
    if (!errors) *serial = sensorSerialNumber;
    else *serial = 0;
    return errors;
}
...
void sht4x_enable_low_power_mode(uint8_t enable_low_power_mode) {
    if (enable_low_power_mode) {
        sht4x_cmd_measure = 0xE0;
        sht4x_cmd_measure_delay_us = 2500;
    } else {
        sht4x_cmd_measure = 0xFD;
        sht4x_cmd_measure_delay_us = 10000;
    }
}

int16_t sht4x_read_serial(uint32_t* serial) {
    const uint8_t cmd = 0x89;
    int16_t errors;
    uint16_t serial_words[(sizeof(*serial) / 2)];

    errors = sensirion_i2c_write(0x44, &cmd, 1);
    if (errors) return errors;

    sensirion_sleep_usec(1000);
    errors = sensirion_i2c_read_words(0x44, serial_words,
                                   (sizeof(serial_words) / 2));
